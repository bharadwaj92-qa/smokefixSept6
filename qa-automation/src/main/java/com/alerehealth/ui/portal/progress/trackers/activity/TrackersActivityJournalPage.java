...
public class TrackersActivityJournalPage extends TrackersActivityBasePage {
	...
	@FindBy(id = "activityJournal_journalEndDate")
	private WebElement journalEndDate;

	@FindBy(id = "activityJournal_journalStartDate")
	private WebElement journalStartDate;

	@FindBy(xpath = "//div[@id='myModal-journalEdit']//p")
	private WebElement editDialougebox;

	@FindBy(xpath = "//div[@id='myModal-journalDelete']//p")
	...
	public String getWeightBmiData(String dateValue,String weightBmiValue){
		...
		String weigthBmiXpath = xpath + "//..//..//following-sibling::div//dt[contains(text(),'"+weightBmiValue+"')]//following-sibling::dd ";
		
		String valueText = " ";
		
		String weightAndBmiValue = " ";
		...
		try {
			...
			if (elementPresent == true) {
				...
				valueText = getWebDriver().findElement(By.xpath(weigthBmiXpath)).getText();
				
				String[] serveValueText1 = weightAndBmiValue.split(" ");
				weightAndBmiValue = serveValueText1[0];
		...
		return weightAndBmiValue;
	
	}

	public String getNotesText(String Date) {

		String xpath = "//div[@class='stacked-table-header clear']//ul//li[not(@id='journal_data_span')][not(@class='bullet')][contains(text(),'"+Date+"')]";
		...
		try {
			...
			if (elementPresent == true) {
				...
				notesText = getWebDriver().findElement(By.xpath(notesXpath)).getText().trim();
	...
	public  TrackersActivityJournalPage clickYesButtonDeleteDialogue() {
	...
	public TrackersActivityTrackPage clickYesButtonEditDialouge() {
...

    {
                    ...
                    List<WebElement> journalEntries = getWebDriver().findElements(By.xpath(("//div[@class='stacked-table-header clear']//ul//li[not(@id='journal_data_span')][not(@class='bullet')][not(@style='float:right')]")));
    ...
    public List<WebElement> getDatesofRecords() {
		...
		String xpath = "//div[@class='stacked-table-header clear']//ul//li[not(@id='journal_data_span')][not(@class='bullet')][not(@style='float:right')]";
	...
	/*
	 * To click journalEntries or GoalHistory dropdown
	 */
	...
	public String getGoalHistoryData(String date, String data){
		
		String xpath = "//div[@class='stacked-table-content']//following-sibling::dd[contains(text(),'"+date+"')]//..//..//dd[contains(text(),'"+data+"')]";
